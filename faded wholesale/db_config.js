  ...
  try {
    ...
    if (typeof Model === 'function') {
      ...
      if (initializedModel && initializedModel.name) {
        ...
        dbLogger.debug(`✅ Model loaded: ${initializedModel.name}`);
      }
...
db.utils = {
  ...
  async initSampleData() {
    ...
    try {
      ...
      dbLogger.info('🌱 Initializing sample data...');
      ...
      dbLogger.info('✅ Sample data initialized successfully');
      return true;
    } catch (error) {
  ...
  async backup(backupPath = './backups') {
    ...
    try {
      ...
      dbLogger.info(`✅ Database backup created: ${fullPath}`);
      return fullPath;
    } catch (error) {
      dbLogger.error('❌ Database backup failed:', error);
      throw error;
    }
  }
};
